...
public class AuthControllerTests : IntegrationTestBase
{
    ...
    public async Task Register_DuplicateEmail_ReturnsBadRequest()
    {
        ...
        var first = new UserRegisterRequest("A", "B", $"userA_{suffix}", email, "Test@1234!");
        var second = new UserRegisterRequest("C", "D", $"userB_{suffix}", email, "Test@1234!");
    ...
    {
        var suffix = Guid.NewGuid().ToString("N")[..10];
        var email = $"forgot_{suffix}@test.com";
        var register = new UserRegisterRequest("F", "G", $"forgot_{suffix}", email, "Test@1234!");
        await CreateAnonymousClient().PostAsJsonAsync("/register", register);
    ...
    {
        var body = new { Email = "nobody_xyz_99999@test.com" };
        var response = await CreateAnonymousClient().PostAsJsonAsync("/forgot-password", body);
    ...
    public async Task ResetPassword_InvalidToken_ReturnsBadRequest()
    {
        ...
        var request = new { Email = email, ResetCode = "totally-invalid-token-xyz", NewPassword = "NewPass@9999!" };
        var response = await CreateAnonymousClient().PostAsJsonAsync("/reset-password", request);
