...
public class NotificationControllerTests : IntegrationTestBase
{
    ...
    [Fact]
    public async Task GetNotifications_Authenticated_ReturnsOkWithEmptyList()
    {
    ...
    public async Task DeleteNotification_ExistingNotification_ReturnsOk()
    {
        ...
        var (guestToken, guestUserId) = await RegisterAndLoginAsync();
        var guestClient = CreateAuthenticatedClient(guestToken);
        var notification = await InviteUserAndGetNotificationAsync(ownerClient, guestClient, board.Id, guestUserId);
