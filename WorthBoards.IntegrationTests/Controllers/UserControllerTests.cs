...
public class UserControllerTests : IntegrationTestBase
{
    ...
    public async Task GetUserById_Unauthenticated_ReturnsUnauthorized()
    {
        ...
        var response = await CreateAnonymousClient().GetAsync($"/api/users/{userId}");
    ...
    public async Task UpdateUser_ValidRequest_ReturnsOk()
    {
        ...
        var (token, userId) = await RegisterAndLoginAsync(suffix);
        var client = CreateAuthenticatedClient(token);
    ...
    public async Task UpdateUser_Unauthenticated_ReturnsUnauthorized()
    {
        ...
        var response = await CreateAnonymousClient().PutAsJsonAsync($"/api/users/{userId}", updateRequest);
